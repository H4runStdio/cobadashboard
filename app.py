...
def run_pipeline(data: dict, tickers: list):
    ...
    try:
        ...
        for i, t in enumerate(tickers):
            pct = 25 + int((i / n) * 40)
            status.markdown(f'<div class="banner-info">🤖 Training model prediksi: <b>{t}</b> ({i+1}/{n})...</div>', unsafe_allow_html=True)
        ...
        progress.progress(65, text="Model prediksi selesai")
